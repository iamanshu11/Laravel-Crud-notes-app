...
@section('content')
    ...
    <form action="{{ route('notes.store') }}" method="POST" class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" class="w-full border border-gray-300 rounded px-3 py-2" value="{{ old('title') }}">
            @error('title')
        ...
        <div class="mb-4">
            ...
            <textarea name="content" rows="5" class="w-full border border-gray-300 rounded px-3 py-2">{{ old('content') }}</textarea>
            @error('content')
                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
